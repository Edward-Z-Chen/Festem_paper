...
## Batch 1
# Load data
adata = read_r("b1_counts_forpython.RData")
counts = adata["counts"]
...
labels1 = scanpy_cluster(X1, features, plot=False, resolution=0.185)
...
file = open('b1_TN_test.pickle', 'wb')
...
#file = open('b1_TN_test.pickle', 'rb')
...
# Result tidy
for i, (c1, c2) in enumerate(itertools.combinations(range(0,6), 2)):
    results[(c1,c2)] = results[(c1,c2)][1]
    
for i, c1 in enumerate(range(0,6)):
    results[(c1)] = results[(c1)][1]

p_frame = pd.DataFrame(results)
p_frame.to_csv("b1_TN_test.csv",index = False,header=False)


## Batch 2
# Load data
adata = read_r("b2_counts_forpython.RData")
counts = adata["counts"]
features = counts.index.values
counts = counts.transpose()
counts = counts.to_numpy()
...
# Generate clusters using X1
labels1 = scanpy_cluster(X1, features, plot=False, resolution=0.2)
...
# open a file, where you ant to store the data
file = open('b2_TN_test.pickle', 'wb')
...
# open a file, where you stored the pickled data
#file = open('b2_TN_test.pickle', 'rb')
...
p_frame = pd.DataFrame(results)
p_frame.to_csv("b2_TN_test.csv",index = False,header=False)