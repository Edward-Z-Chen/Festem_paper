...
B <- 20
num.core <- 12
save.image("./results/NB_400DE_5type_workspace.RData")
...
for (i in 1:1){
  ...
  save(counts,mu.list,log.foldchange,sigma.list,sigma.list2,file = paste0("./results/counts/NB_400DE_5type_",i,".RData"))
  ...
  if (i == B | i == 1){
    pdf("./results/NB_400DE_5type_UMAP_initial.pdf",width=4,height=4,onefile = T)
  ...
  save(adjpvalue.list,time.mat,cluster.acc,cluster.labels.mat,total.memory.usage,peak.memory.usage,file = "./results/NB_400DE_5type_DEG.RData")
  ...
  save(FS.gene.list,FS.time.mat,file = "./results/NB_400DE_5type_FS.RData")
  ...
  save(FS.gene.list,FS.time.mat,FS.ARI.frame,FS.DB.frame,FS.label.list,FS.SI.frame,
       umap.list,plots.list,total.memory.usage,peak.memory.usage,file = "./results/NB_400DE_5type_FS.RData")
  gc(verbose = F)
  ...
  save(FS.gene.list,FS.time.mat,FS.ARI.frame,FS.DB.frame,FS.label.list,FS.SI.frame,
       ...
       file = "./results/NB_400DE_5type_FS.RData")
  gc(verbose = F)
  
  rm(list = ls())
  load("./results/NB_400DE_5type_workspace.RData")
