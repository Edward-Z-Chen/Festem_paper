...
for (i in 1:length(gene.list)){
  ...
  tsne.list[[i]] <- RunTSNE(ifnb, reduction = "pca", dims = 1:20)@reductions[["tsne"]]
...
save(gene.list,label.list,umap.list,plots.list,file = "./results/ifnb_ctrl_clustering_tSNE.RData")
...
for (i in 1:length(gene.list)){
  ...
  ifnb <- ScaleData(ifnb,features = gene.list[[i]])
  ifnb <- RunPCA(ifnb, verbose = FALSE,features = gene.list[[i]])
  ifnb <- FindNeighbors(object = ifnb, dims = 1:15)
  for (j in 1:15){
    ifnb <- FindClusters(object = ifnb, resolution = 0.05*j+0.75)
