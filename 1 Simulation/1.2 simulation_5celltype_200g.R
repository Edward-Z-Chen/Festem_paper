...
B <- 20
num.core <- 12
save.image("./results/NB_200DE_5type_workspace.RData")
...
for (i in 1:B){
  ...
  save(counts,mu.list,log.foldchange,sigma.list,sigma.list2,file = paste0("./results/counts/NB_200DE_5type_",i,".RData"))
  ...
  if (i == B | i == 1){
    pdf("./results/NB_200DE_5type_UMAP_initial.pdf",width=4,height=4,onefile = T)
  ...
  save(adjpvalue.list,time.mat,cluster.acc,cluster.labels.mat,total.memory.usage,peak.memory.usage,file = "./results/NB_200DE_5type_DEG.RData")
  ...
  time.mat[i,"Festem_0.05_f"] <- time.tmp$Elapsed_Time_sec*num.core+time.mat[i,"Festem_0.05"]
  ...
  snowparam <- SnowParam(workers = num.core, type = "SOCK", log = T)
  ...
  time.mat[i,"Festem_7group_0.05"] <- time.tmp$Elapsed_Time_sec*num.core
  ...
  time.mat[i,"Festem_7group_0.05f"] <- time.tmp$Elapsed_Time_sec*num.core+time.mat[i,"Festem_7group_0.05"]
  ...
  time.mat[i,"Festem_9group_0.05"] <- time.tmp$Elapsed_Time_sec*num.core
  ...
  time.mat[i,"Festem_9group_0.05f"] <- time.tmp$Elapsed_Time_sec*num.core+time.mat[i,"Festem_9group_0.05"]
  ...
  save(FS.gene.list,FS.time.mat,FS.ARI.frame,FS.DB.frame,FS.label.list,FS.SI.frame,
       umap.list,plots.list,total.memory.usage,peak.memory.usage,file = "./results/NB_200DE_5type_FS.RData")
  gc(verbose = F)
  ...
  gene.list <- list(FS.gene.list[[1]][[i]][1:200],
                    ...
                    head(FS.gene.list[[5]][[i]],200),
                    head(FS.gene.list[[6]][[i]],200),
                    FS.gene.list[[7]][[i]][1:200],
                    FS.gene.list[[8]][[i]][1:200],
  ...
  save(FS.gene.list,FS.time.mat,FS.ARI.frame,FS.DB.frame,FS.label.list,FS.SI.frame,
       ...
       file = "./results/NB_200DE_5type_FS.RData")
  gc(verbose = F)
  
  rm(list = ls())
  load("./results/NB_200DE_5type_workspace.RData")
